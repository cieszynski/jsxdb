...
import JSxUtils from "http://127.0.0.1:8081/jsxutils.mjs";
        ...
        try {
            ...
            if (!dbname) {
                ...
                if (!storename) {
                    ...
                    // pathname: "/jsxdb{/:dbname}?{/:storename}"
                    if (!itemid) {
                        switch (method) {
                            case "GET":
                                const queryobj = JSxUtils.parseSearchParams(
                                    url,
                                );
                                ...
                                Object.assign(response, {
                                    ...
                                    status: 200,
                                    result: await store.parse(queryobj),
                                });
                                break;
                        }
                        db.close();
                        ...
                        const id = JSxUtils.intOrString(itemid);
                        ...
                        // pathname: "/jsxdb{/:dbname}?{/:storename}?{/:itemid}"
                        if (!subpath) {
                            switch (method) {
                                case "GET":
                                    Object.assign(response, {
                            ...
                            switch (method) {
                                case "GET":
                                    response.result = subpath.split("/").reduce(
                                        (prev, curr) => {
                                            return prev[curr];
                                        },
                                        item,
